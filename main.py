...
def extract_video(url: str = Query(..., description="The social media video URL to extract")):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # 1. Prioritize a combined format containing both video and audio
            for f in formats:
                if f.get("vcodec") != "none" and f.get("acodec") != "none" and f.get("url"):
                    video_link = f.get("url")
                    break

            # 2. Fallback to highest resolution video-only track
            if not video_link:
                video_formats = [f for f in formats if f.get("vcodec") != "none" and f.get("url")]
                if video_formats:
                    video_link = video_formats[-1].get("url")

            # 3. Isolate standalone audio track if separate
            for f in formats:
                if f.get("vcodec") == "none" and f.get("acodec") != "none" and f.get("url"):
                    audio_link = f.get("url")
                    break

            # Final check: Fallback to root url if formatting looks skipped it
            if not video_link:
            ...
            if video_link:
                ...
                response_data["images"] = False  # Fixed layout schema flag as requested
